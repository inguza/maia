...
normalize_output() {
    ...
    local GITBASHWJQ="$GITBASHW"
    local GITBASHH="$GITBASHW"
    local GITBASHHJQ="$GITBASHW"
    if command -v cygpath > /dev/null 2>&1 ; then
	GITBASHW=$(cygpath -w "$M_ROOT")
	GITBASHWJQ=${GITBASHW//\\//}
	GITBASHW=${GITBASHW//\\/\\\\}
	GITBASHH=$(cygpath -w "$XMAIA_HOME")
	GITBASHHJQ=${GITBASHH//\\//}
	GITBASHH=${GITBASHH//\\/\\\\}
    fi
    sed "
...
s|$GITBASHH|<MAIA_HOME>|g;
s|$GITBASHHJQ|<MAIA_HOME>|g;
s|$M_ROOT|<MAIA_ROOT>|g;
s|$GITBASHW|<MAIA_ROOT>|ig;
s|$GITBASHWJQ|<MAIA_ROOT>|ig;
