...
**NOTE!** If you use Git for Windows, be aware that its default line-ending configuration will check out files using Windows-style `\r\n` line endings.

A clone using Windows-style line endings will not work from WSL or another Linux environment expecting Unix-style line endings. Use separate clones for Git Bash and WSL/Linux, or configure Git to use Unix-style line endings consistently before sharing a clone between environments.
